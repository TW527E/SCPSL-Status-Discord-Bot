...
if(jdata['Server 1'][0]['Switch']):
    bot1 = commands.Bot(command_prefix='\|/1', intents = intents)

    @bot1.event
    async def on_ready():
        counter = 0
        await bot1.change_presence(activity=discord.Streaming(name="伺服器狀態機器人 已上線", url="https://www.twitch.tv/tw527e"))
        if counter == 0:
    ...
    async def invite(ctx):
        ...
        await ctx.send("伺服器邀請連結:https://discord.gg/vCTtKazpZz")
...
if(jdata['Server 2'][0]['Switch']):
    ...
    async def on_ready():
        counter = 0
        await bot2.change_presence(activity=discord.Streaming(name="伺服器狀態機器人 已上線", url="https://www.twitch.tv/tw527e"))
        if counter == 0:
...
if(jdata['Server 3'][0]['Switch']):
    ...
    async def on_ready():
        counter = 0
        await bot3.change_presence(activity=discord.Streaming(name="伺服器狀態機器人 已上線", url="https://www.twitch.tv/tw527e"))
        if counter == 0:
        ...
        if(jdata['Server 1'][0]['Switch']):
            ...
            if(jdata['Do not change this']["Success"]):
                if(jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Online"]):
                    ...
                    embed=discord.Embed(title=f"{jdata[f'Server {Server_Number}'][0]['Server Name']} {jdata[f'Server {Server_Number}'][0]['Server Number']} 目前狀態", description=f"{jdata[f'Server {Server_Number}'][0]['Server Description']}", color=0x28d252)
                    ...
                    embed.set_footer(text=F"此資料上次更新時間 {datetime.datetime.now()} • Last updated {datetime.datetime.now()} (GMT+8)", icon_url=bot1.get_user(356361726427660288).avatar_url)
                else:
                    Port = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Port"]
                    LastOnline = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["LastOnline"]
                    ...
                    embed=discord.Embed(title=f"{jdata[f'Server {Server_Number}'][0]['Server Name']} {jdata[f'Server {Server_Number}'][0]['Server Number']} 目前狀態", description=f"{jdata[f'Server {Server_Number}'][0]['Server Description']}", color=0xFF0000)
                    embed.set_thumbnail(url=f"{jdata[f'Server {Server_Number}'][0]['Server Icon']}")
                    embed.add_field(name="＝＝＝＝＝＝＝＝＝＝＝", value="狀態－Status", inline=False)
            ...
            print(F"[Server #{Server_Number}][{datetime.datetime.now()}] 伺服器資料已更新")
        ...
        if(jdata['Server 2'][0]['Switch']):
            ...
            if(jdata['Do not change this']["Success"]):
                if(jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Online"]):
                    ...
                    embed.add_field(name="最後上線日期", value=F"{LastOnline}", inline=True)
                    embed.add_field(name="Last Online Date", value=F"{LastOnline}", inline=True)
                    embed.set_footer(text=F"此資料上次更新時間 {datetime.datetime.now()} • Last updated {datetime.datetime.now()} (GMT+8)", icon_url=bot2.get_user(356361726427660288).avatar_url)
                else:
                    Port = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Port"]
                    LastOnline = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["LastOnline"]
        ...
        if(jdata['Server 3'][0]['Switch']):
            ...
            if(jdata['Do not change this']["Success"]):
                if(jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Online"]):
                    ...
                    embed.add_field(name="最後上線日期", value=F"{LastOnline}", inline=True)
                    embed.add_field(name="Last Online Date", value=F"{LastOnline}", inline=True)
                    embed.set_footer(text=F"此資料上次更新時間 {datetime.datetime.now()} • Last updated {datetime.datetime.now()} (GMT+8)", icon_url=bot3.get_user(356361726427660288).avatar_url)
                else:
                    Port = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["Port"]
                    LastOnline = jdata['Do not change this']["Servers"][jdata[f'Server {Server_Number}'][0]['Server Codename']]["LastOnline"]
